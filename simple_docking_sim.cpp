...
// ==============================================================================
//  RIGID BODY POST-CONTACT POSE SIMULATION - PES UNIVERSITY
//  Guide: Dr. Adithya Balasubramanyam (CAVE Labs)
// ==============================================================================
...
struct Quat {
    ...
    // Convert to Roll-Pitch-Yaw in degrees (for console output matching Table 3.6)
...
// ------------------------------------------------------------------------------
// 3. RIGID BODY STATE (Section 2.4 & 3.2)
// ------------------------------------------------------------------------------
struct RigidBody {
    std::string name;
    ...
    Quat attitude;      // Orientation quaternion [w, x, y, z] (Section 1.6)
...
// ------------------------------------------------------------------------------
// 3. LOAD CELL CONFIGURATION (Section 3.2)
// 6 Load cells mounted on regular hexagon vertices, radius R = 0.5 m, on contact face
// ------------------------------------------------------------------------------
...
// ------------------------------------------------------------------------------
// 4. LOAD CELL FORCE GENERATOR (PHYSICAL FORWARD MODEL)
// Distributes physical contact force F applied at point P_c across the 6 load cells.
// ------------------------------------------------------------------------------
...
// ------------------------------------------------------------------------------
// 5. POST-CONTACT POSE ESTIMATION (Section 2.5 & 2.6 - RECONSTRUCTION FROM SENSORS)
// Equations from Dr. Adithya's paper:
//   F_net = sum(F_i)
...
// ------------------------------------------------------------------------------
// 7. SIMULATION SCENARIO MANAGEMENT
// ------------------------------------------------------------------------------
enum class Scenario {
...
bool g_paused = false;
Vec3 g_visualContactPt(0, 0, 0);
bool g_showContactMarker = false;

void SetupScenario(Scenario sc) {
    g_currentScenario = sc;
    g_contactOccurred = false;
    g_showContactMarker = false;
    ...
    g_target.attitude = Quat(1.0, 0.0, 0.0, 0.0); // Identity quaternion (no initial rotation)
    ...
    g_chaser.attitude = Quat(1.0, 0.0, 0.0, 0.0); // Identity quaternion
...
int main() {
    ...
    std::cout << "  Fully-Coupled Physical Sensor & Post-Contact Pose Simulation\n";
    std::cout << "===============================================================\n";
    ...
    GLFWwindow* window = glfwCreateWindow(1100, 700, "Rigid Body Docking Simulation - PES University", NULL, NULL);
    if (!window) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!g_paused) {
            ...
            if (!g_contactOccurred && isTouching) {
                ...
                // Contact lasts over discrete window dt = 2 ms (Section 3.1)
                ...
                // 3. Sensor Model: Physical force distributes to the 6 load cells on the hexagon
                Vec3 loadCellForces[6];
                DistributeContactForceToLoadCells(totalContactForce, physicalContactPt, loadCellForces);

                // 4. Dr. Adithya's Paper Reconstruction (Section 2.5 & 2.6)
                ...
                // Compute Quaternion representation of contact rotation (Section 1.6)
                ...
                std::cout << "True Physical Contact Pt:(" << physicalContactPt.x << ", " << physicalContactPt.y << ", " << physicalContactPt.z << ") m\n";
                std::cout << "---------------------------------------------------------------\n";
                std::cout << "6 LOAD CELL READINGS (from hexagon distribution):\n";
                for (int i = 0; i < 6; ++i) {
                    std::cout << "  Cell " << (i+1) << ": (" 
                ...
                std::cout << "---------------------------------------------------------------\n";
                std::cout << "ESTIMATION FROM PAPER FORMULAS (Section 2.5 - 2.6):\n";
            ...
            // Continuous motion in zero-gravity (Section 3.5)
